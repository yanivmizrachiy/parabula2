...
REPORT="DELTOID_GRAPHICS_SYSTEM_ADVANCE_$(date +%Y%m%d_%H%M%S).md"
...
enhanced_nums = {p.stem.replace("page-","").replace("-enhanced","") for p in enhanced_assets}
vector_nums = set()

for p in vector_pages:
    stem = p.stem
    num = stem.replace("page-","").replace("-vector","")
    vector_nums.add(num)

all_exact_nums = sorted({p.stem.replace("page-","") for p in exact_pages})
