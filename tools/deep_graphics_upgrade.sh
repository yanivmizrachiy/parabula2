...
python "$PY" > /tmp/deltoid_graphics_analysis.json
cp /tmp/deltoid_graphics_analysis.json worksheets/deltoid/exact_assets/graphics_analysis.json
...
for p in $UPGRADE_PAGES; do
  printf "Upgrading page %s\n" "$p"

  pdftoppm -f "$p" -l "$p" -png -r 400 "$PDF" "$ASSET_DIR/rebuild-$p"

  if [ -f "$ASSET_DIR/rebuild-$p-$p.png" ]; then
    mv -f "$ASSET_DIR/rebuild-$p-$p.png" "$ASSET_DIR/page-$(printf '%02d' "$p")-enhanced.png"
  elif [ -f "$ASSET_DIR/rebuild-$p-$(printf '%02d' "$p").png" ]; then
    mv -f "$ASSET_DIR/rebuild-$p-$(printf '%02d' "$p").png" "$ASSET_DIR/page-$(printf '%02d' "$p")-enhanced.png"
  fi

  "$IMGTOOL" "$ASSET_DIR/page-$(printf '%02d' "$p")-enhanced.png" \
    -colorspace sRGB \
    -strip \
    -filter Lanczos \
    -resize 2480x3508 \
    ...
    "$ASSET_DIR/page-$(printf '%02d' "$p")-enhanced.png"

  cat > "$PAGE_DIR/page-$(printf '%02d' "$p").html" <<EOF
<!DOCTYPE html>
...
<body>
  <div class="exact-shell">
    <main class="a4-page exact-page exact-page-enhanced">
      <img class="exact-image exact-image-enhanced" src="../exact_assets/page-$(printf '%02d' "$p")-enhanced.png" alt="דלתון עמוד $p משופר">
    </main>
  </div>
</body>
</html>
...

{
  ...
  cat worksheets/deltoid/exact_assets/graphics_analysis.json
  echo
